...
void calcSpectrumStatistics(float &lowFreq, float &highFreq) {
    float length = static_cast<float>(transmitedMessage.bits_count);

    lowFreq = lowestHarmonicSpin.getValue() / length * 5.0f;
    highFreq = highestHarmonicSpin.getValue() / length * 5.0f;
}
...
static void setSpectrumStatistics() {
    ...
    calcSpectrumStatistics(lowFreq, highFreq);
...
static std::string getNameForScreenshot() {
    // lowestHarmonicSpin.getValue();
    // highestHarmonicSpin.getValue();
    ...
    print_format(name, sizeof(name), "%s Screenshot Spectrum=%d_%d,harmonics=%2.1f_%2.1f,%s,%s,noise=%1.2f,desync=%1.2f,voltage=%1.2f",
                                     PROGRAM_NAME,
                                     lowestHarmonicSpin.getValue(), highestHarmonicSpin.getValue(),
                                     lowFreq, highFreq,
                                     std::regex_replace(physMethodsRadioGroup.getSelectedButton().getText(), std::regex("/"), "").c_str(),
                                     std::regex_replace(logicMethodsRadioGroup.getSelectedButton().getText(), std::regex("/"), "").c_str(),
                                     noiseSpin.getValue(), desyncSpin.getValue(), voltageSpin.getValue());

    return std::string(name);
}

static void takeScreenshot(const sf::RenderWindow &window) {
    //std::cout << getNameForScreenshot() << std::endl;
    ...
    const std::string rawScreenshotFilename = getNameForScreenshot();
    std::string screenshotFilename = rawScreenshotFilename + ".png";

    nfdchar_t *outPath;
    nfdfilteritem_t filters[1] = { { "Images", "jpg,png,tiff,bmp" } };
    const nfdchar_t *defaultPath = getExternalPath().c_str();
    const nfdchar_t *defaultName = screenshotFilename.c_str();
    ...
    if (altFileSave) {
        screenshotFilename = rawScreenshotFilename;
